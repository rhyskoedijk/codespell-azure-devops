...
import { AzureDevOpsClient, IPullRequstLock } from './utils/azureDevOpsClient';
import { CodespellRunner } from './utils/codespellRunner';
import { parseExtensionConfiguration } from './utils/extensionConfigParser';
...
async function run() {
  ...
  try {
    if (config.skipIfCodeSpellConfigMissing && !config.hasCodeSpellConfigFile) {
      console.info(
        "Skipping task because '.codespellrc' configuration file is missing and 'skipIfCodeSpellConfigMissing' is set.",
      );
    ...
    if (config.pullRequestId > 0) {
      ...
      if (!prLock.wasAcquired || prLock.ownerJobId !== config.jobId) {
        console.info(
          `Skipping task because another instance of codespell is already running for this PR in ${prLock.ownerJobId ? 'job ' + prLock.ownerJobId : 'another job'}.`,
        );
