...
export class CodespellRunner {
  ...
  public async installIfMissing(): Promise<void> {
    ...
    const pipRunner: ToolRunner = tool(which('pip', true)).arg(['install', 'codespell', 'chardet']);
    pipRunner.execSync({
      silent: !this.debug,
    });
  }
  ...
  // Run codespell
  public async run(options: { writeChanges: boolean; postFixCommand?: string }): Promise<ICodeSpellResult> {
    ...
    if (suggestions.length > 0) {
      warning(
        `Found ${suggestions.length} misspelling(s) ${options.writeChanges ? 'that could not be automatically corrected' : ''}:`,
      );
