...
import { IFileSuggestion } from './types';
...
export class CodespellCommandProcessor {
  ...
  public getCommandHelpTextFor(suggestion: IFileSuggestion): string {
    return [
      ...
      ' - `' +
        this.commandPrefix +
        ' ignore this` will ignore this single misspelling instance using an inline code comment',
      ' - `' +
        this.commandPrefix +
        ' ignore line` will ignore all misspellings on line ' +
        suggestion.lineNumber +
        ' using an inline code comment',
      ...
      ' - `' +
        this.commandPrefix +
        ' ignore file-type` will add `*.' +
        suggestion.path.split('.').pop() +
        '` to the global ignored files list',
      ' - `' +
        this.commandPrefix +
        ' ignore dir` will add `' +
        suggestion.path.split(path.sep).splice(0, -1).join(path.sep) +
        path.sep +
        '*` to the global ignored files list',
      ' - `' +
        this.commandPrefix +
        ' ignore <pattern>` will add a custom file path pattern to the global ignored files list',
      '',
  ...
  public async processCommands(options: {
    ...
    // Parse the command
    const command: string[] = options.comment.content
      .substr(this.commandPrefix.length)
      .trim()
      .split(' ')
  ...
  private async processIgnoreCommand(
    ignoreTarget: string,
    options: {
      suggestion: IFileSuggestion;
    },
    ...
    switch (ignoreTarget) {
      case 'this':
        ...
        return patchFileWithInlineComment(
          options.suggestion.path,
          options.suggestion.lineNumber,
          options.suggestion.word,
        );
      ...
      case 'dir':
        ...
        return patchCodespellConfig(
          this.configFilePath,
          `${options.suggestion.path.split(path.sep).splice(0, -1).join(path.sep)}${path.sep}*`,
          null,
        );
...
function patchCodespellConfig(
  configPath: string,
  addSkipPattern: string | null = null,
  addIgnoreWord: string | null = null,
): boolean {
  try {
...
function patchFileWithInlineComment(filePath: string, lineNumber: number, word: string | null = null): boolean {
  try {
    ...
    const codespellIgnoreComment =
      `${codeCommentDelimiters.start} ${codespellIgnore} ${codeCommentDelimiters.end || ''}`.trim();
