import { getBoolInput, getInput, getVariable } from 'azure-pipelines-task-lib/task';
import fs from 'fs';
import ini from 'ini';
...
export function parseExtensionConfiguration(): IExtensionConfig {
  ...
  return {
    ...
    commitSuggestions:
      getBoolInput('commitSuggestions', false) || codeSpellDevOpsConfig?.['commit-suggestions'] !== undefined || false,
    commentSuggestions:
      getBoolInput('commentSuggestions', false) ||
      codeSpellDevOpsConfig?.['comment-suggestions'] !== undefined ||
      false,
    ...
    failOnMisspelling:
      getBoolInput('failOnMisspelling', false) || codeSpellDevOpsConfig?.['fail-on-misspelling'] !== undefined || false,
    debug:
      getVariable('System.Debug')?.toLowerCase() === 'true' || codeSpellDevOpsConfig?.['debug'] !== undefined || false,
  };
}
